...
# run 2 tests
gtimeout -s SIGTERM 5s ./client a config.txt < ${curr_folder}input1.txt > ${curr_folder}output1.log 2>&1
gtimeout -s SIGTERM 5s ./client a config.txt < ${curr_folder}input2.txt > ${curr_folder}output2.log 2>&1
